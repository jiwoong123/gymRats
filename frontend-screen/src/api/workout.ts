...
export type CalendarWorkoutBrief = {
  id: number;
  name: string;
  started_at: string;
  duration: number;
...
export type WorkoutCalendarDay = {
  date: string;
  body_parts: number[];
  workouts: CalendarWorkoutBrief[];
};
export type MuscleFatigueStat = { body_part: number; fatigue: number; last_trained_at: string | null };
export type BodyPartWorkoutStat = { body_part: number; completed_sets: number; volume: number };
export type WeeklyWorkoutStat = { week_start: string; sessions: number; completed_sets: number };
export type WorkoutStatistics = {
  ...
  muscle_fatigue: MuscleFatigueStat[];
  body_parts: BodyPartWorkoutStat[];
  weekly: WeeklyWorkoutStat[];
};
...
export async function getWorkoutStatistics() {
  const response = await api.get<WorkoutStatistics>("/workout-sessions/statistics");
  return response.data;
}
