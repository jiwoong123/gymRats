...
export default function Home() {
  ...
  const weeklyStats = [
    ...
    { label: "볼륨", value: formatVolume(data.weekly_summary.volume), unit: "kg" },
    { label: "세트", value: String(data.weekly_summary.sets), unit: "세트" },
  ];
  const displayedQuickWorkouts = data.quick_workouts.slice(0, 6);
    ...
    <div className="home-page">
      ...
      <section className="section home-section">
        <div className="section-header">
          ...
          >
            루틴 관리 <ChevronRight size={14} />
          </button>
        </div>
        {data.quick_workouts.length > 0 ? (
          <div className="quick-grid">
            {displayedQuickWorkouts.map((workout) => (
              <button
                key={workout.routine_id}
                className="quick-btn"
                onClick={() => navigate(`/workout-session?routine_id=${workout.routine_id}`)}
              >
                <span className="quick-emoji"><RoutineIcon name={workout.icon} size={20} /></span>
        ...
        {data.recent_workouts.length > 0 ? (
          <div className="recent-list">
            {data.recent_workouts.map((workout) => (
              <div key={workout.id} className="recent-item" onClick={() => navigate(`/records/workouts/${workout.id}`)}>
                ...
                <div className="recent-volume">
                  ...
                  <span className="recent-vol-unit">kg</span>
                </div>
              </div>
            ))}
          </div>
